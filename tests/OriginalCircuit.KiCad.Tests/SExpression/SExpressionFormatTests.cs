...
/// <summary>
/// Tests for S-expression format fidelity: OriginalText, WasCompact, indent detection,
...
public class SExpressionFormatTests
{
    ...
    // ---------------------------------------------------------------
    // Full round-trip: parse S-expression text, write back, compare
    // ---------------------------------------------------------------

    [Fact]
    public void RoundTrip_SimpleKiCadSymbolLibFragment_BytePerfect()
    {
        // Test a simple structure where all children are on their own lines
        // (no mixed inline/multiline like property nodes)
    ...
    public async Task RoundTrip_MinimalSymbolLibFile_SourceTreeIsStored()
    {
        ...
        var lib = await OriginalCircuit.KiCad.Serialization.SymLibReader.ReadAsync(path);
    ...
    public async Task RoundTrip_MinimalSymbolLibFile_NumbersPreserved()
    {
        ...
        await OriginalCircuit.KiCad.Serialization.SymLibWriter.WriteAsync(lib, ms);
